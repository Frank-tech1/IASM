...
class VoxelMap:
    def __init__(self, cfg, bbox, device):
        ...
        # [新增 1] 语义对数几率层 (Semantic Log-Odds Layer) [cite: 42]
        # 用于存储当前目标(如"chair")的语义存在概率
        self.voxel_sem_lo = torch.zeros(torch.prod(dim), device=device) 

    # ... (保留 update_utility 等原有几何方法) ...

    # [新增 2] 贝叶斯语义更新 (Bayesian Semantic Update) 
    def update_semantics(self, visible_indices, detection_confidence):
        """
        根据 VLM 的观测结果更新语义地图
        visible_indices: 当前视锥内的体素索引
        detection_confidence: VLM 认为这些体素属于目标的置信度 (0~1)
        """
        if len(visible_indices) == 0:
            return

        # 将置信度转换为 Log-Odds 增量
        # 观测模型：如果 VLM 说看到了，我们给一个正向的 Log-Odds 奖励
        # 如果没看到，给一个负向的惩罚 (可选)
        
        # 简单传感器模型：
        # p_hit = detection_confidence (e.g., 0.8)
        # log_odds_update = log(p / (1-p))
        
        # 为了数值稳定性，clip 概率
        conf = np.clip(detection_confidence, 0.1, 0.9)
        update_val = np.log(conf / (1 - conf))
        
        # 执行公式: L(t) = L(t-1) + Update(z_t) 
        # 注意：这里我们假设 visible_indices 是所有被检测框覆盖的体素
        self.voxel_sem_lo[visible_indices] += update_val
        
        # 截断以防止数值溢出 (Clamp)
        self.voxel_sem_lo = torch.clamp(self.voxel_sem_lo, min=-5.0, max=5.0)

    # [新增 3] 获取语义概率 P(S_v | Cmd) [cite: 73]
    @property
    def semantic_probabilities(self):
        return self.inverse_log_odds(self.voxel_sem_lo)

    # [新增 4] 计算语义熵 H_sem(v) [cite: 73]
    def get_semantic_entropy(self):
        probs = self.semantic_probabilities

...
from einops import rearrange
from tqdm import tqdm
import numpy as np # Added numpy import which was missing in snippet but used in add_gaussians
from utils.operations import * # Assuming imports are correct from your environment
from utils.common import TextColors
from .utils import (
    l1_loss_fc_mask,
    normal_tv_loss_fc,
...
# Assuming necessary imports for renderer are available
...
class GaussianMap:
    def __init__(self, cfg, device):
        ...
        # [IASM] 新增属性：语义特征与动态标记
        # 128维 = 32(粗) + 48(中) + 48(细)
        self._semantic_features = torch.empty((0, 128), device=device)
        # 动态标记: 0.0 (静态) -> 1.0 (动态)
        self._dynamic_mask = torch.empty((0, 1), device=device)
    ...
    def train(self, steps=None):
        ...
        for i in tqdm(
            ...
            # ... (Training loop kept mostly same, semantic loss can be added later) ...
            [rgb_gts, depth_gts, extrinsics, intrinsics], frame_ids = (
                training_sampler.next_frames(self.training_performance)
            )
            *_, h, w = rgb_gts.shape
            
            # Note: For now we only train geometry/RGB. 
            # Semantic training requires modifying the Renderer to output semantic maps.
            # We skip adding semantic loss here to keep Phase 1/2 simple.
    ...
    def post_processing(self):
        ...
        if require_prune:
            extrinsics = torch.stack([self.training_data[i]["extrinsic"] for i in range(num_training_frame)])
            intrinsics = torch.stack([self.training_data[i]["intrinsic"] for i in range(num_training_frame)])
            # rgb_gts = torch.stack([self.training_data[i]["rgb"] for i in range(num_training_frame)])
            depth_gts = torch.stack([self.training_data[i]["depth"] for i in range(num_training_frame)])
        else:
            extrinsics = self.training_data[-1]["extrinsic"].unsqueeze(0)
            intrinsics = self.training_data[-1]["intrinsic"].unsqueeze(0)
            # rgb_gts = self.training_data[-1]["rgb"].unsqueeze(0)
        ...
        depth_ranges = self.training_data[-1]["depth_range"]
        # using h,w from last frame
        _, h, w = self.training_data[-1]["rgb"].shape 
        
        (
            _, _, _, _, _, _, _, counts, _,
        ) = GaussianRenderer(
            extrinsics,
            intrinsics,
            self.get_attr(),
        ...
        if self.use_view_distribution:
            ...
            view_directions = view_directions / view_distances.unsqueeze(-1)  # normalize
            delta = view_directions[update_mask] - self.view_means[update_mask]

            # view mean vectors
            self.view_means[update_mask] += delta / self.view_supports[update_mask].unsqueeze(-1)

            # view scores
            cosine_sim = torch.clamp(torch.sum(gaussian_normals * view_directions, 1), min=0, max=1)
            distance_factor = torch.clamp(view_distances / depth_ranges[1], min=0, max=1)
            self.view_scores[update_mask] += (1 - distance_factor)[update_mask] * cosine_sim[update_mask]

        if require_prune:
            vis_mask = (torch.sum(counts, dim=0) >= 1.0)
            self.prune(~vis_mask)
    ...
    def prune(self, prune_mask):
        ...
        # [IASM] Pruning: remove semantic and dynamic attributes too
        self._means = self._means[~prune_mask]
        self._scales = self._scales[~prune_mask]
        self._rotations = self._rotations[~prune_mask]
        self._opacities = self._opacities[~prune_mask]
        ...
        self._semantic_features = self._semantic_features[~prune_mask]
        self._dynamic_mask = self._dynamic_mask[~prune_mask]
    ...
    def init_training(self):
        # We assume semantic features are NOT optimized via gradient descent in this version
        # to save memory and maintain 30Hz. They are updated via direct assignment/fusion.
        self._means = nn.Parameter(self._means)
        self._scales = nn.Parameter(self._scales)
        self._rotations = nn.Parameter(self._rotations)
        self._opacities = nn.Parameter(self._opacities)
        self._harmonics = nn.Parameter(self._harmonics)
        l = [
            {"params": [self._means], "lr": self.cfg.optimizer.mean_lr, "name": "mean"},
            {"params": [self._scales], "lr": self.cfg.optimizer.scale_lr, "name": "scale"},
            {"params": [self._rotations], "lr": self.cfg.optimizer.rotation_lr, "name": "rotation"},
            {"params": [self._opacities], "lr": self.cfg.optimizer.opacity_lr, "name": "opacity"},
            {"params": [self._harmonics], "lr": self.cfg.optimizer.harmonic_lr, "name": "harmonic"},
        ]
        self.optimizer = torch.optim.Adam(l, eps=1e-15)
    ...
    def add_gaussians(self, dataframe):
        ...
        # [IASM] 获取语义输入
        # 默认 None，如果是 None 则初始化为 0
        input_semantic = dataframe.get("semantic_features", None) 
        ...
        if self.is_init:
            # (Rendering logic kept same for brevity, assuming GaussianRenderer works)
            (
                rgb_pred, depth_pred, normal_pred, opacity_pred, _, confidence_pred, _, _, _,
            ) = GaussianRenderer(
                extrinsic.unsqueeze(0).to(self.device),
                intrinsic.unsqueeze(0).to(self.device),
                self.get_attr(),
        ...
        # [IASM] Initialize Semantic Features & Dynamic Mask
        # 1. 语义: 初始化为 0 或输入值
        semantic_new = torch.zeros(point_num, 128, device=self.device)
        if input_semantic is not None:
            # 假设 input_semantic 是 [1, 32] (粗特征)，我们将其放入前 32 维
            # 广播到所有点: (1, 32) -> (point_num, 32)
            coarse_dim = input_semantic.shape[-1]
            semantic_new[:, :coarse_dim] = input_semantic.repeat(point_num, 1)
        
        # 2. 动态标记: 默认为 0 (静态)
        # 如果 FastStream 提供了 pixel-wise dynamic mask，可以在这里赋值
        dynamic_mask_new = torch.zeros(point_num, 1, device=self.device)
        ...
        # Mask Calculation
        select_mask = self.cal_mask(
            rgb.unsqueeze(0), depth.unsqueeze(0), global_render_results
        )
        select_mask = select_mask.to(self.device).squeeze(0) * valid_mask
        selected_idx = torch.nonzero(select_mask, as_tuple=False).flatten()
        ...
        test_mask = torch.zeros(len(selected_idx), dtype=torch.bool)
        selected_pcd = pcd[select_mask]
        if len(selected_pcd) > 0: # Safety check
            vf_idx = voxel_downsample(selected_pcd.to(self.device))
            test_mask[vf_idx] = True
            selected_idx = selected_idx[test_mask]
            select_mask_final[selected_idx] = True
        ...
        # Concatenation (Appending new Gaussians)
        self._means = torch.cat((self._means.detach(), means_new.float()[select_mask]), dim=0)
        self._scales = torch.cat((self._scales.detach(), scales_new.float()[select_mask]), dim=0)
        self._harmonics = torch.cat((self._harmonics.detach(), harmonics_new.float()[select_mask]), dim=0)
        self._opacities = torch.cat((self._opacities.detach(), opacities_new.float()[select_mask]), dim=0)
        self._rotations = torch.cat((self._rotations.detach(), rotations_new.float()[select_mask]), dim=0)

        # [IASM] Concatenate Semantic & Dynamic
        self._semantic_features = torch.cat(
            (self._semantic_features.detach(), semantic_new.float()[select_mask]), dim=0
        )
        self._dynamic_mask = torch.cat(
            (self._dynamic_mask.detach(), dynamic_mask_new.float()[select_mask]), dim=0
        )

        self.view_scores = torch.cat((self.view_scores, view_scores_new.float()[select_mask]), dim=0)
        self.view_supports = torch.cat((self.view_supports, view_supports_new.float()[select_mask]), dim=0)
        self.view_means = torch.cat((self.view_means, view_means_new.float()[select_mask]), dim=0)
    ...
    def save(self, save_path, index="final"):
        map_state = {
            ...
            "opacities": self._opacities.detach(),
            "rotations": self._rotations.detach(),
            # [IASM] Save new attributes
            "semantic_features": self._semantic_features.detach(),
            "dynamic_mask": self._dynamic_mask.detach(),
    ...
    def load(self, model_path):
        ...
        # [IASM] Load new attributes
        if "semantic_features" in map_state:
            self._semantic_features = map_state["semantic_features"]
        else:
            # Backwards compatibility
            self._semantic_features = torch.zeros((self._means.shape[0], 128), device=self.device)
            
        if "dynamic_mask" in map_state:
            self._dynamic_mask = map_state["dynamic_mask"]
        else:
            self._dynamic_mask = torch.zeros((self._means.shape[0], 1), device=self.device)
